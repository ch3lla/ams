...
const updateCourse = async (req: any, res: any) => {
    ...
    try {
        const filteredData = Object.fromEntries(
            Object.entries(course_data).filter(([_, value]) => value !== null && value !== undefined)
        );
        ...
        if (!updatedCourse) {
            return res.status(404).json({ message: "Course not found or you do not have permission to update this course." });
        }
...
const removeCourse = async (req: any, res: any) => {
    ...
    try {
        const deletedCourse = await Course.findOneAndDelete({_id: course_id, lecturer: _id});
